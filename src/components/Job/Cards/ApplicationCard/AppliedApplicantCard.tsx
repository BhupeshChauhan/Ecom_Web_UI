...
export const AppliedApplicantCard = ({ jobDetails }: any) => {
  const {
    name,
    designation,
    applicantId,
  ...
  const { isLoading, SendApplicantInvite } = JobService();
  const [ImageFallback, setImageFallback] = useState(profileImageUrl);
  const { handleReload } = UseDataGrid();
  return (
    <div className="flex flex-col text-left px-3 py-0 pt-8 w-full bg-white">
      ...
      <div className="flex items-start justify-start pt-4 text-sm">
        ...
        <div className="flex flex-col items-end gap-2 w-[20%] text-sm">
          <Button
            ...
            onClick={async () => {
              await SendApplicantInvite({
                jobId: location.pathname.split("/")[2],
                applicationId: applicationId,
              });
              handleReload(
                `${import.meta.env.VITE_API_JOB_BASE_URI}/jobs/applicants/${location.pathname.split("/")[2]}?status=applied`,
                token,
              );
            }}
          >
          ...
          <Link
            to={`/jobs/${location.pathname.split("/")[2]}/applicant/edit/${applicationId}`}
            className="w-full"
          >
            <Button variant="outline" fullwidth={true}>
              Edit Applicant
