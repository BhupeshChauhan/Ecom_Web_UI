...
export const ShortListedApplicantCard = ({ jobDetails }) => {
  const {
    name,
    designation,
    id,
    ...
    <div className="flex flex-col text-left px-3 py-0 pt-8  w-full">
      ...
      <div className="flex items-start justify-start pt-4 text-sm">
        ...
        <div className="flex flex-col items-end gap-2 w-[20%] text-sm">
          <Button variant="ghost">Review Notes</Button>
          <Link
            to={`/jobs/${location.pathname.split("/")[2]}/assessment/report/${interviewId}?status=shortlisted`}
            className="w-full"
          >
